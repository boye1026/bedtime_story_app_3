...
"""
通义千问 AI 服务模块
使用 OpenAI SDK 兼容格式连接通义千问 API，提供故事生成能力，支持流式和非流式响应
"""
...
class AIService:
    ...
    def init_app(self, app):
        ...
        logger.info("AI 服务初始化完成，模型: %s", self.model)
    ...
    def _generate_sync(self, system_prompt, user_prompt):
        ...
        try:
            ...
            logger.info("故事生成完成，长度: %d 字符", len(content))
            return content
            ...
            raise AIServiceError(f"AI 服务调用失败: {str(e)}")
    ...
    def _generate_stream(self, system_prompt, user_prompt):
        ...
        try:
            ...
            logger.info("流式故事生成完成")

        except Exception as e:
            logger.error("AI API 流式调用失败: %s", str(e))
            raise AIServiceError(f"AI 流式服务调用失败: {str(e)}")
